...
func (w *worker) commitNewWork(interrupt *int32, noempty bool, timestamp int64) {
	...
	bundles, err := w.eth.TxPool().SealingBundles(header.Number)
	if err != nil {
		log.Error("Failed to fetch pending bundles","err",err)
		return
	}
	// Short circuit if there is no available pending transactions
	if len(pending) == 0 && len(bundles) == 0 {
		w.updateSnapshot()
		return
	}
	start := time.Now()
	...
	if len(bundles) > 0 {
		splitBundle := w.splitBundle(bundles)
		if w.commitBundle(splitBundle, w.coinbase, interrupt) {
			return
		}
		log.Info("Inserted bundled transaction(s)","block",header.Number,"count",len(splitBundle))
	}
...
func (w *worker) splitBundle(bundles []types.Transactions) (types.Transactions) {
	returnBundle := types.Transactions{}
